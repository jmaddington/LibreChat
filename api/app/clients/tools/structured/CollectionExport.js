    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        doc
          .fillColor('#7f8c8d')
          .fontSize(12)
          .text(`Export Date: ${formatDate(new Date())}`, { align: 'center' });
        doc.moveDown(1.5);
        ...
        doc
          .fillColor('#2c3e50')
          .rect(50, doc.y + 2, doc.page.width - 100, 2)
          .fill();
        doc.moveDown(0.8);
        ...
        if (data.collections && data.collections.length > 1) {
          ...
          doc
            .fillColor('#16a085')
            .rect(50, doc.y + 2, doc.page.width - 100, 1)
            .fill();
          doc.moveDown(0.8);
          ...
          data.collections.forEach((collection) => {
            if (collection.id !== data.collection.id) {
              ...
              doc
                .fillColor('#ecf0f1')
                .rect(60, boxY - 5, doc.page.width - 120, 60)
                .fill();
        ...
        if (data.notes && data.notes.length > 0) {
          ...
          data.notes.forEach((note, index) => {
            ...
            doc
              .fillColor('#f8f9fa')
              .rect(50, noteHeaderY - 5, doc.page.width - 100, 35)
              .fill();
            doc
              .fillColor('#2c3e50')
              .fontSize(13)
            ...
            doc
              ...
              .font('Helvetica')
              .text(
                `Collection: ${data.collections.find((c) => c.id === note.collection_id)?.name || 'Unknown'}`,
                60,
              )
              .text(`Created: ${formatDate(note.created_at)}`, 60);

            if (note.source_url) {
              doc.fillColor('#007bff').fontSize(9).text(`Source: ${note.source_url}`, 60);
            }
            ...
            // Content section
            doc.fillColor('#495057').fontSize(10).font('Helvetica-Bold').text('Content:', 60);
            doc
              .fillColor('#212529')
              .fontSize(11)
              .font('Helvetica')
            ...
            doc
              .fillColor('#dee2e6')
              .rect(50, doc.y, doc.page.width - 100, 3)
              .fill();
            doc.moveDown(1);
          });
        }
