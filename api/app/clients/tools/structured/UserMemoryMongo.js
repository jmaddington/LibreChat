...
class UserMemoryMongo extends Tool {
  constructor(fields = {}) {
    ...
    this.description_for_model = `Manages persistent user memories with optional tagging system. Allows agents to store and retrieve information about users across sessions.
    ...
    this.schema = z.object({
      ...
      content: z
        ...
        .max(10000)
        .optional()
        .describe('Memory content for create/update operations'),

      tags: z
        .array(z.string().min(1).max(100))
        .max(50)
        .optional()
        .describe('Tag names to associate with memory'),

      tag_filter: z
        .array(z.string().min(1).max(100))
        .max(50)
        .optional()
        .describe('Filter memories by these tag names'),
      ...
      tag_name: z
        ...
        .max(100)
        .optional()
        .describe('Tag name for create/update tag operations'),
    ...
    try {
      ...
      if (tagFilter && tagFilter.length > 0) {
        ...
        if (sanitizedTags.length > 0) {
          ...
          const tagRegex = new RegExp(`(^|_)(${sanitizedTags.join('|')})(_|$)`, 'i');
          query.key = { $regex: tagRegex };
        }
      }

      const memories = await this.models.MemoryEntry.find(query).sort({ updated_at: -1 }).lean();
  ...
  extractTagsFromKey(key) {
    ...
    const filteredParts = parts.filter((part) => part.length > 2 && part !== 'memory');

    const partsWithoutSuffix = filteredParts.slice(0, -1);

    // Return the parts that might be tags (last few components)
    return partsWithoutSuffix.slice(-3); // Return last 3 as potential tags
  }
  ...
  async _call(input) {
    try {
      ...
      const { action, memory_id, content, tags, tag_filter, tag_id, tag_name, new_tag_name } =
      ...
      switch (action) {
        ...
        case 'update_memory':
          if (!memory_id || !content) {
            return JSON.stringify({
              error: 'memory_id and content are required for update_memory',
            });
          }
